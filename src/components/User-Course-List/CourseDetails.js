...
const CourseDetails = () => {
    ...
    const fetchCourseDetails = async () => {
      ...
      try {
        const response = await fetch(
          `http://127.0.0.1:5000/api/course-details/${courseId}?user_id=${userId}`
        );
        if (!response.ok) {
  ...
  const handleResourceCompletion = async (resourceId, moduleId) => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:5000/api/complete-resource", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <button className="course-details-back-button" onClick={handleBackButtonClick}>
      ...
      {courseDetails.modules.map((module) => (
        <div key={module.module_id} className="course-details-module-item">
          ...
          {/* <div className="course-details-quiz-link">
          <a href={`/indexquiz.html`} className="course-details-quiz-button" target="_blank">
              Ai Powered Quiz
          </a>
          ...
          {/* <div className="course-details-quiz-link">
          <a href={`/memory-game${module.module_title[0]}.html`} className="course-details-quiz-button" target="_blank">
              Play Game for {module.module_title}
