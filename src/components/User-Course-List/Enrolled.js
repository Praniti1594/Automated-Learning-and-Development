...
const Enrolled = () => {
  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:5000/api/enroll?user_id=${userId}`);
        if (!response.ok) {
  ...
  const handleCreateGoal = (courseId) => {
    navigate(`/goalplan/${courseId}`); // Navigate to course details page
  };
...
>
  Create Personalised Study Plan
</a>
