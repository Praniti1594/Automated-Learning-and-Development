...
def send_email(to_email, subject, body):
    sender_email = "pranitikubal9@gmail.com"  
    sender_password = "prih znmp sivs uiya"   
...
def fetch_detailed_course(course_id):
    ...
    enrollment = UserCourse.query.filter_by(user_id=user_id, course_id=course_id, status='enrolled').first()
    if not enrollment:
...
# Route to fetch modules for a course
@course_blueprint.route('/api/generateplan/<int:course_id>/modules', methods=['GET'])
def get_modules(course_id):
    try:
        # Fetch modules for the given course_id, ordered by order_no
        modules = Module.query.filter_by(course_id=course_id).order_by(Module.order_no).all()
        
        # Convert modules to a list of dictionaries
        modules_list = [
            {
                'module_id': module.module_id,
                'module_title': module.module_title,
