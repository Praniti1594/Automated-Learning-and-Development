...
def user_progress_overview():
    ...
    # Get participants with role_id 4
    participants = User.query.filter_by(role_id=4).all()

    for participant in participants:
        # Join Progress with Module and filter by course_id at the DB level
        query = db.session.query(Progress, Module).join(Module, Progress.module_id == Module.module_id).filter(Progress.user_id == participant.user_id)
...
def user_progress(user_id):
    ...
    query = db.session.query(Progress, Module).join(Module, Progress.module_id == Module.module_id).filter(Progress.user_id == user_id)
    if course_id:
        query = query.filter(Module.course_id == course_id)
